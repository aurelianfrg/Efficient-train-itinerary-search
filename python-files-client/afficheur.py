...
class Afficheur(tk.Tk):
  ...
  def afficher_resultats_recherche(self, itineraire):
    ...
    if itineraire == "CONNEXION ERROR":
      self.ecrire_text_area("Connexion au serveur impossible.")
      return 

    self.ecrire_text_area("Itinéraire trouvé avec toutes les correspondances:" + "\n\n", clear=True)
   
    self.ecrire_text_area(str(itineraire))
    
    if itineraire == []:  #pas d'itineraire trouvé
      self.ecrire_text_area(f"Aucun itinéraire n'a pu être trouvé.", clear = True)



  def afficher_itineraire_carte(self, itineraire):
    if self.carte:
      ...
      for trajet in itineraire:
        self.map_widget.set_path( [(float(dico_stops[trajet[0]][1]),(float(dico_stops[trajet[0]][2]))), (float(dico_stops[trajet[1]][1]), (float(dico_stops[trajet[1]][2])))] , width=5)
